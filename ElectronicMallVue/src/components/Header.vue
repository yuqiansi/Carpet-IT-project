...
<!--    页签-->
    <el-breadcrumb style="display: inline-block; margin-left: 30px;font-size: 22px">
      <el-breadcrumb-item :to="{ path: '/manage/home' }">Home Page</el-breadcrumb-item>
    ...
    <span class="el-dropdown-link">
      ...
      <el-dropdown-menu slot="dropdown" style="text-align: center">
        <el-dropdown-item>
          <div @click="$router.push('/manage/person')">Personal Information</div>
        </el-dropdown-item>
        <el-dropdown-item>
...
export default {
  ...
  data(){
    return{
      ...
      // user: localStorage.getItem("user") ? JSON.parse(localStorage.getItem("user")) : {nickname:'Login is invalid'}
    }
  },
  watch: {
    //面包屑
  ...
  created() {
    // this.user = localStorage.getItem("user") ? JSON.parse(localStorage.getItem("user")) : {nickname:'Login is invalid'};
    this.routePath=this.$route.meta.path;
  }
}
